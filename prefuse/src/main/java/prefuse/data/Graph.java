...
public class Graph extends CompositeTupleSet {
    ...
    protected static final String INDEGREE  = PrefuseLib.FIELD_PREFIX + "indegree";
    /** Out-degree data field for the links table */
    protected static final String OUTDEGREE = PrefuseLib.FIELD_PREFIX + "outdegree";
    /** In-links adjacency list data field for the links table */
    protected static final String INLINKS   = PrefuseLib.FIELD_PREFIX + "inlinks";
    /** Out-links adjacency list data field for the links table */
    protected static final String OUTLINKS  = PrefuseLib.FIELD_PREFIX + "outlinks";
    ...
    private int m_addingEdgePosInSourceNode = ADDING_EDGE_END;
    private int m_addingEdgePosInTargetNode = ADDING_EDGE_END;
    ...
    protected void updateDegrees(int e, int s, int t, int incr) {
        if ( incr > 0 ) {
            ...
            remLink(OUTLINKS, OUTDEGREE, s, e);
            remLink(INLINKS, INDEGREE, t, e);
        }
     ...
     * @param edgesColName which adjacency list (inlinks or outlinks) to use
     * @param degreeColName the length of the adjacency list, alse is the degree corresponding to the node n
     * @param n the node id of the adjacency list to use
     * @param e the edge to add to the list
     */
    protected void addLink(String edgesColName, String degreeColName, int n, int e, int edgePosInNode) {
    	Table nodeTable = getNodeTable();
    	
        int[] array = (int[])nodeTable.get(n, edgesColName);
        int degree = nodeTable.getInt(n, degreeColName);
        
        if ( array == null ) {
            array = new int[] {e};
            nodeTable.set(n, edgesColName, array);
        } else {
        ...
        nodeTable.setInt(n, degreeColName, degree+1);
    }
     ...
     */
    protected boolean remLink(String edgesColName, String degreeColName, int n, int e) {
    	Table nodeTable = getNodeTable();
        ...
        for ( int i=0; i<degree; ++i ) {
            if ( array[i] == e ) {
                System.arraycopy(array, i+1, array, i, degree-i-1);
		        nodeTable.setInt(n, degreeColName, degree-1);
    ...
    private void setEdgeNode (Table edges, int e, String sourceOrTargetKey, int n)
    {
        // get keys for the nodes
        long key = getKey(n);
        
        if (m_longKey) {
            edges.setLong(e, sourceOrTargetKey, key);
        }
        else {
            edges.setInt(e, sourceOrTargetKey, (int)key);
        }
    }
     ...
     * @param edgePosInSourceNode 
     * @param edgePosInTargetNode
     * @return the edge id (edge table row number) of the added edge
     */
    public int addEdge(int s, int t, int edgePosInSourceNode, int edgePosInTargetNode) {
    	
        m_addingEdgePosInSourceNode = edgePosInSourceNode;
        m_addingEdgePosInTargetNode = edgePosInTargetNode;
        
        // add edge row, set source/target fields
        Table edges = getEdgeTable();
        int r = edges.addRow();
        
        setEdgeNode (edges, r, m_skey, s);
        setEdgeNode (edges, r, m_tkey, t);
        return r;
    }
    
    public int addEdge (int s, int t) {
    	return addEdge (s, t, ADDING_EDGE_END, ADDING_EDGE_END);
    }
     ...
     */
    public Edge addEdge(Node s, Node t, int edgePosInSourceNode, int edgePosInTargetNode) {
        nodeCheck(s, true);
        nodeCheck(t, true);
        int e = addEdge(s.getRow(), t.getRow(), edgePosInSourceNode, edgePosInTargetNode);
        return getEdge(e);
    }
    ...
    public boolean removeNode(int node) {
        ...
        if ( nodeTable.isValidRow(node) ) {
            ...
            if ( id > 0 ) {
                int[] links = (int[])nodeTable.get(node, INLINKS);
                for ( int i=id; --i>=0; )
                    removeEdge(links[i]);
            }
            int od = getOutDegree(node);
            if ( od > 0 ) {
                int[] links = (int[])nodeTable.get(node, OUTLINKS);
                for ( int i=od; --i>=0; )
                    removeEdge(links[i]);
            }
        }
    ...
    public void changeEdgeSource(int e, int newSource, int edgePosInSourceNode)
    {
        m_listener.setEnabled(false);
        
        int oldSource = getSourceNode(e);
        
    	remLink (OUTLINKS, OUTDEGREE, oldSource, e);
    	addLink (OUTLINKS, OUTDEGREE, newSource, e, edgePosInSourceNode);
    	
    	setEdgeNode(getEdgeTable(), e, m_skey, newSource);
         
        m_listener.setEnabled(true);
    }
    
    public void changeEdgeSource(Edge e, Node newSource, int edgePosInSourceNode)
    {
        edgeCheck(e, true);
        nodeCheck(newSource, true);
        changeEdgeSource(e.getRow(), newSource.getRow(), edgePosInSourceNode);
    }
    
    public void changeEdgeSource(int e, int newSource)
    {
    	changeEdgeSource(e, newSource, ADDING_EDGE_END);
    }
    
    public void changeEdgeSource(Edge e, Node newSource)
    {
    	changeEdgeSource(e, newSource, ADDING_EDGE_END);
    }
    ...
    public int getInDegree(int node) {
        return getNodeTable().getInt(node, INDEGREE);
    }
    ...
    public int getOutDegree(int node) {
        return getNodeTable().getInt(node, OUTDEGREE);
    }
    ...
    public int getEdge(int source, int target) {
        ...
        if ( outd > 0 ) {
            int[] edges = (int[])getNodeTable().get(source, OUTLINKS);
    ...
    public IntIterator edgeRows(int node, int direction) {
        if ( direction==OUTEDGES ) {
            int[] outedges = (int[])getNodeTable().get(node, OUTLINKS);
            return new IntArrayIterator(outedges, 0, getOutDegree(node));
        } else if ( direction==INEDGES ) {
            int[] inedges = (int[])getNodeTable().get(node, INLINKS);
